...
with open('./message/randomtext/randomno2.txt', 'r', encoding='utf-8') as hiddenfile:
    lyrics_content = hiddenfile.read()

marked_lyrics = lyrics_content + "#####"

hidden_data = np.frombuffer(
    marked_lyrics.encode('utf-8'),
    dtype=np.uint8
)

print("Hidden text byte array shape:", hidden_data.shape)
